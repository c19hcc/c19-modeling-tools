    ...
    def run(self, chains=1, tune=3000, draws=1000, target_accept=.95, prev_param_mat = None): 
        ...
        with pm.Model() as model:
            ...
            if prev_param_mat is None:
                ...
                vec = pm.MvNormal('cov_vector', mu = np.array(prev_param_mat[0:3]), cov = cov_mat, shape = 3)
...
def create_and_run_model(name, state, p_delay, prev_param = None):
    confirmed = state.positive.diff().dropna()
    onset = confirmed_to_onset(confirmed, p_delay)
    adjusted, cumulative_p_delay = adjust_onset_for_right_censorship(onset, p_delay)
    window = len(confirmed.index)
    if prev_param is None:
        return MCMCModel(name, onset, cumulative_p_delay, window).run()
    else: 
        prev_param_mat = prev_param.loc[:,'si_mean':'cov_mat6'].values.tolist()[0]
        #print(prev_param_mat)
        return MCMCModel(name, onset, cumulative_p_delay, window).run(prev_param_mat = prev_param_mat)
...
def run_national_model(state_filename = 'data/state_inf_stats.csv', overlap = False, p_delay = None):
    ...
    if overlap:
        prev_data = pd.read_csv('{}/data/rt_calcs/data/rt_national.csv'.format(data_dir), 
            parse_dates=['date'])
        prev_data.to_csv('{}/data/rt_calcs/data/rt_national_prev.csv'.format(data_dir), index=True)
        prev_param = prev_data.iloc[[-14]]
        prev_date = prev_param.iloc[0]['date']

        nat_cts = nat_cts[(nat_cts.index >= prev_date)]

        nat_model = create_and_run_model(region, nat_cts, p_delay, prev_param)
...
def run_states_model(filename = 'data/state_inf_stats.csv', overlap = False, p_delay = None):
...
def run_counties_model(filename = 'data/county_inf_stats.csv', overlap = False, p_delay = None):
...
#python3 rt_calc_scipt.py [region] [overlap? T/F]
# If overlap is true, re-calculate the past 14 days from previous dataframe
if __name__ == '__main__':
    ...
    if len(sys.argv) in [2,3]:
        if sys.argv[1] in ['US', 'states', 'counties']:
            ...
            raise Exception('Please enter a valid argument one of two: US, states, or counties')
        if sys.argv[2] in ["TRUE"]:
            overlap = True
        else:
            raise Exception('Please enter a valid argument two of two: "TRUE" (optional)')
    else: 
        raise Exception('Please enter a valid argument: US, states, or counties; optionally followed by TRUE to use past data')
    
    p_delay = read_patient_data('{}/data/rt_calcs/data/linelist.csv'.format(data_dir))
    
    if model_type == 'US':
        run_national_model('{}/data/database/state_inf_stats.csv'.format(data_dir), overlap, p_delay)
    elif model_type == 'states':
        run_states_model('{}/data/database/state_inf_stats.csv'.format(data_dir), overlap, p_delay)
    else: 
        run_counties_model('{}/data/database/county_inf_stats.csv'.format(data_dir), overlap, p_delay)
